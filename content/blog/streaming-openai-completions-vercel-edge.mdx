...
date: 01-14-2023
image: /blog/streaming-openai-completions-vercel-edge.jpg
---

I recently built an AI-powered development tool ([Refraction](https://www.refraction.dev)) using the OpenAI API. I was really impressed with the results of OpenAI and Vercel, so I wanted to share how I built both tools in less than 2 days.
...
```ts showLineNumbers title="pages/api/generate.ts"
...
```ts showLineNumbers title="pages/api/generate.ts" {32,45,47-49}
import type { NextRequest } from 'next/server';
...
```tsx showLineNumbers title="pages/generate.tsx"
...
```tsx showLineNumbers title="pages/generate.tsx" {29,37-40,44,65-67}
const [text, setText] = useState('');
